     ...
     startclient)
        ...
        if [ $STANDALONE = 0 ]; then
            if [ $VNC_NEW_VERSION = 1 ]; then
               /sbin/daemonize.sh "vncviewer" "$2 -ViewOnly -FullScreen -UseLocalCursor=0 -passwd $3"
            ...
            if [ $VNC_NEW_VERSION = 1 ]; then
               $DBUS_HANDLER --auth=$4 --type=exec --text="vncviewer $2 -ViewOnly -FullScreen -UseLocalCursor=0 -passwd $3"
            else
