...
{
	printf("Usage: tmixer-alsa <options> [command]\n");
	printf("\nAvailable options:\n");
			     ...
			     char **ptr, int dir)
{
	...
	if (*p == '%') {
		...
		p++;
		/* FIXME tmixer-alsa conversion lose original % because of 0-39 limits */
		/*printf("DEBUG: pmin=%li pmax=%li val=%li\n",pmin,pmax,val);*/
	} else if (p[0] == 'd' && p[1] == 'B') {
		if (! invalid) {
...
static int show_selem(snd_mixer_t *handle, snd_mixer_selem_id_t *id, const char *space, int level)
{
	...
	long pmin = 0, pmax = 0;
	long cmin = 0, cmax = 0;
    // Allow control capture channels for rtp, mic..etc
	long pvol, cvol;
	//long pvol;
	int psw, csw;
	//int psw;
	...
	if ( level & LEVEL_BASIC ) {
	    ...
	    printf("%s,", snd_mixer_selem_id_get_name(id) );
		/*printf("%sCapabilities:", space);*/
		
		if( snd_mixer_selem_has_common_volume(elem) || snd_mixer_selem_has_playback_volume(elem) || snd_mixer_selem_has_capture_volume(elem) ) {
		    printf("volume|");
		}
		if ( snd_mixer_selem_has_common_switch(elem) || snd_mixer_selem_has_playback_switch(elem) || snd_mixer_selem_has_capture_switch(elem) ) {
		    printf("switch|");
		}
		...
		if (pmono && snd_mixer_selem_has_capture_channel(elem, SND_MIXER_SCHN_MONO)) {
			int title = 0;
			if (!mono_ok) {
				/*printf("%s%s:", space, "Mono");*/
				mono_ok = 1;
		...
		if (!pmono || !cmono) {
			...
			if (snd_mixer_selem_has_playback_channel(elem, chn)) {
				...
				if (snd_mixer_selem_has_capture_channel(elem, chn)) {
					...
					if (!pmono && snd_mixer_selem_has_capture_channel(elem, chn)) {
						int title = 0;
						if (!snd_mixer_selem_has_common_volume(elem)) {
							if (snd_mixer_selem_has_capture_volume(elem)) {
								/*printf(" Playback");*/
	    ...
	    if ( strlen(snd_mixer_selem_id_get_name(id)) > 0 ) {
	        printf("%s|", snd_mixer_selem_id_get_name(id) );
	    }
	}
...
static int parse_control_id(const char *str, snd_ctl_elem_id_t *id)
{
	...
	while (*str) {
		if (!strncasecmp(str, "numid=", 6)) {
			...
			numid = atoi(str);
			if (numid <= 0) {
				fprintf(stderr, "tmixer-alsa: Invalid numid %d\n", numid);
				return -EINVAL;
			}
...
static int sset_channels(snd_mixer_elem_t *elem, unsigned int argc, char **argv)
{
	...
	for (idx = 1; idx < argc; idx++) {
		...
		/*printf("DEBUG: argc=%d, arvg[0]=%s, argv[1]=%s\n", argc, argv[0], argv[1]);*/
		...
		for (chn = 0; chn <= SND_MIXER_SCHN_LAST; chn++) {
			...
			if ((dir & 1) && snd_mixer_selem_has_playback_channel(elem, chn)) {
				...
				if (!strncmp(ptr, "mute", 4) && snd_mixer_selem_has_playback_switch(elem)) {
				    ...
				    /* tmixer-alsa FORCE to work always with % */
					ptr=strcat(ptr, "%");
...
int tmixer_alsa(int argc, char *argv[]) {
#else
int main(int argc, char *argv[]) {
	...
	while (1) {
		...
		case 'n':
			...
			break;
		case 'v':
			printf("tmixer-alsa version (TCOS) " TCOS_VERSION "\n");
			return 1;
		case 'a':
			smixer_level = 1;
	...
	if (!strcmp(argv[optind], "help")) {
		...
		fprintf(stderr, "tmixer-alsa: Unknown command '%s'...\n", argv[optind]);
	}
